...
def blur_page():
    # Page title and project introduction
    st.markdown("## Blur")
    st.markdown(
        "Launched on October 19, 2022, Blur is an NFT marketplace and aggregator. "
        "Users can compare NFTs across marketplaces, manage portfolios with advanced analytics, as well as buy NFTs. "
        "The VC-backed digital marketplace stated that users can sweep and snipe NFTs faster than on other platforms, "
        "and is 10x faster than the aggregator Gem."
    )
    st.markdown("---")
    ...
    for item in json_top_collection:
        if (item["PLATFORM_NAME"] == 'blur' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if (item['PLATFORM_NAME'] == 'blur' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        ...
        chart_collection_count = alt.Chart(df_collection_count).mark_bar(color='#E4831E').encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar(color='#E4831E').encode(
    ...
    with daily_sales_count:
        ...
        daily_sales_count_area = daily_sales_count_base.mark_area(color='#E4831E').encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(
            x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area(color='#E4831E').encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area(color='#E4831E').encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    with daily_buyer_seller:
        daily_buyer_seller_color = {'Buyer': '#E4831E', 'Seller': '#7FE0FE'}
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line(color='#E4831E').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar(color='#E4831E').encode(
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar(color='#E4831E').encode(
    ...
    for item in json_trader_share:
        if(item['BLOCK_DATE'] >= '2022-10-19'):
            save_dict_trader_share["Date"].append(item["BLOCK_DATE"])
            save_dict_trader_share["% of Total Traders on Ethereum"].append(item["BLUR_SHARE"])
