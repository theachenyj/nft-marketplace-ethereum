...
def looksrare_page():
    st.markdown("## Looksrare")
    st.markdown(
        "Launched in January 2022 by \"Zodd\" and \"Guts\", "
        "LooksRare is a decentralized NFT marketplace that runs on Ethereum. "
        "Users can list, buy, sell, stake and trade NFTs as well as earn rewards "
        "in the form of LOOKS tokens. Developers can use the LooksRare API to retrieve data on orders "
        "and access other contracts. Looksrare's reward system is an incentive for users of the platform."
    )
    st.markdown("---")
    ...
    for item in json_data_total:
        if(item['PLATFORM_NAME'] == 'looksrare'):
    ...
    for item in json_top_collection:
        if (item["PLATFORM_NAME"] == 'looksrare' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if (item['PLATFORM_NAME'] == 'looksrare' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        ...
        chart_collection_count = alt.Chart(df_collection_count).mark_bar(color='#74CD61').encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar(color='#74CD61').encode(
    ...
    with daily_sales_count:
        ...
        daily_sales_count_area = daily_sales_count_base.mark_area(color='#74CD61').encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(
            x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area(color='#74CD61').encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area(color='#74CD61').encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    with daily_buyer_seller:
        daily_buyer_seller_color = {'Buyer': '#74CD61', 'Seller': '#7FE0FE'}
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line(color='#74CD61').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar(color='#74CD61').encode(
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar(color='#74CD61').encode(
    ...
    for item in json_trader_share:
        if (item['BLOCK_DATE'] >= '2022-01-04'):
            save_dict_trader_share["Date"].append(item["BLOCK_DATE"])
            save_dict_trader_share["% of Total Traders on Ethereum"].append(item["LOOKSRARE_SHARE"])
    ...
    with trader_share:
        trader_share_chart = alt.Chart(df_trader_share).mark_area(color='#74CD61').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
