...
def nftx_page():
    st.markdown("## NFTX")
    st.markdown(
        "Founded in 2020, NFTX is a NFT marketplace that allows users to create liquid markets for otherwise illiquid NFTs. "
        "Users can deposit one of their NFTs into a vault and mint an ERC-20 token. This token (vToken) represents a claim on some asset in that vault and "
        "can then be used in turn to redeem NFTs from the vault. "
        "This allows collectors to earn a better yield on their non-fungible investments as well as better distribution and price discovery for NFT projects in general."
    )
    st.markdown("---")
...
# Top 10 NFT Collections
    ...
    for item in json_top_collection:
        if (item["PLATFORM_NAME"] == 'nftx' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if (item['PLATFORM_NAME'] == 'nftx' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        ...
        chart_collection_count = alt.Chart(df_collection_count).mark_bar(color='#D62C7B').encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar(color='#D62C7B').encode(
    ...
    with daily_sales_count:
        ...
        daily_sales_count_area = daily_sales_count_base.mark_area(color='#D62C7B').encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(
            x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area(color='#D62C7B').encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area(color='#D62C7B').encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    with daily_buyer_seller:
        daily_buyer_seller_color = {'Buyer': '#D62C7B', 'Seller': '#7FE0FE'}
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line(color='#D62C7B').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar(color='#D62C7B').encode(
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar(color='#D62C7B').encode(
    ...
    for item in json_trader_share:
        if (item['BLOCK_DATE'] >= '2021-06-28'):
            save_dict_trader_share["Date"].append(item["BLOCK_DATE"])
            save_dict_trader_share["% of Total Traders on Ethereum"].append(item["NFTX_SHARE"])
    ...
    with trader_share:
        trader_share_chart = alt.Chart(df_trader_share).mark_area(color='#D62C7B').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
