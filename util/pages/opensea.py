...
def opensea_page():
    st.markdown("## Opensea")
    st.markdown(
        "Founded by Devin Finzer and Alex Atallah in 2017, OpenSea is a decentralized marketplace for the purchase and sale of NFTs. "
        "Users can buy, collect, trade and gift NFTs on the marketplace. "
        "To date, OpenSea is the most popular and widely used NFT marketplace."
    )
    st.markdown("---")
    ...
    for item in json_top_collection:
        if(item["PLATFORM_NAME"] == 'opensea' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if(item['PLATFORM_NAME'] == 'opensea' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        df_collection_count = pd.DataFrame(data=save_dict_top_collection_count, columns=['NFT Collection', 'Sales Count'])
        chart_collection_count = alt.Chart(df_collection_count).mark_bar().encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar().encode(
        ...
        """
        chart_collection_volume = alt.Chart(df_collection_volume).mark_arc(innerRadius=50, tooltip=True).encode(
            theta=alt.Theta(field="Sales Volume", type="quantitative", stack= "normalize"),
            color=alt.Color(field="NFT Collection", type="nominal"),
        ).properties(title='Top 10 NFT Collections Based on Volume')
        """
    ...
    df_daily_sales_count = pd.DataFrame(data=save_dict_daily_sales_count, columns=['Date', 'Sales Count', 'Ether Price'])
    ...
    df_daily_sales_volume = pd.DataFrame(data=save_dict_daily_sales_volume, columns=['Date', 'Sales Volume', 'Ether Price'])

    daily_sales_count = st.container()
    with daily_sales_count:
        daily_sales_count_base = alt.Chart(df_daily_sales_count).encode(x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_count_area = daily_sales_count_base.mark_area().encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area().encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area().encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    df_daily_buyer_seller = pd.DataFrame(data=save_dict_daily_buyer_seller, columns=['Date', 'Trader Type', 'Traders'])
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line().encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    df_trader_group_sales_count = pd.DataFrame(data=save_dict_trader_group_sales_count, columns=['Sales Count', '% of Total Traders', 'Rankings'])
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar().encode(
            x=alt.X("Sales Count:N", sort=alt.SortField(field='Rankings', order='ascending'), axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar().encode(
            x=alt.X("Sales Volume:N", sort=alt.SortField(field='Rankings', order='ascending'), axis=alt.Axis(title=None)),
            y=alt.Y("% of Total Traders:Q", axis=alt.Axis(format='%')),
            tooltip=[
    ...
    for item in json_trader_share:
        ...
        save_dict_trader_share["% of Total Traders on Ethereum"].append(item["OPENSEA_SHARE"])
    ...
    with trader_share:
        trader_share_chart = alt.Chart(df_trader_share).mark_area().encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
