...
def rarible_page():
    st.markdown("## Rarible")
    st.markdown(
        "Founded in 2020 by Alex Salnikov, Rarible is an Ethereum-based platform for creating, selling and buying NFTs. "
        "Users can acquire crypto assets on Rarible. Developers can use Rarible Multichain Protocol to mint NFTs, "
        "create sell orders and bids as well as purchase an NFT. The platform has a large base of 1.6 million users and a venture capital of $16 million."
    )
    st.markdown("---")
...
# Top 10 NFT Collections
    ...
    for item in json_top_collection:
        if (item["PLATFORM_NAME"] == 'rarible' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if (item['PLATFORM_NAME'] == 'rarible' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        ...
        chart_collection_count = alt.Chart(df_collection_count).mark_bar(color='#F4D834').encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar(color='#F4D834').encode(
    ...
    with daily_sales_count:
        ...
        daily_sales_count_area = daily_sales_count_base.mark_area(color='#F4D834').encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(
            x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area(color='#F4D834').encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area(color='#F4D834').encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    with daily_buyer_seller:
        daily_buyer_seller_color = {'Buyer': '#F4D834', 'Seller': '#7FE0FE'}
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line(color='#F4D834').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar(color='#F4D834').encode(
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar(color='#F4D834').encode(
    ...
    for item in json_trader_share:
        if (item['BLOCK_DATE'] >= '2020-11-17'):
            save_dict_trader_share["Date"].append(item["BLOCK_DATE"])
            save_dict_trader_share["% of Total Traders on Ethereum"].append(item["RARIBLE_SHARE"])
    ...
    with trader_share:
        trader_share_chart = alt.Chart(df_trader_share).mark_area(color='#F4D834').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
