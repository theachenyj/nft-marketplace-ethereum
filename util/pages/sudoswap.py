...
def sudoswap_page():
    st.markdown("## Sudoswap")
    st.markdown(
        "Launched in May 2022, Sudoswap is a decentralized exchange protocol that runs on Ethereum. "
        "Users can swap NFTs in a trustless environments as well as create and trade through liquidity pools. "
        "Developers can launch NFT collections on Sudoswap or integrate Sudoswap into programmes using the Sudoswap AMM. "
        "Sudoswap charges a low fee of 0.5%. "
    )
    st.markdown("---")
    ...
    for item in json_data_total:
        if(item['PLATFORM_NAME'] == 'sudoswap'):
...
# Top 10 NFT Collections
    ...
    for item in json_top_collection:
        if (item["PLATFORM_NAME"] == 'sudoswap' and item['COUNT_RANKINGS'] <= 10):
    ...
    for item in json_top_collection:
        if (item['PLATFORM_NAME'] == 'sudoswap' and item['VOLUME_RANKINGS'] <= 10):
    ...
    with top_collections_count:
        ...
        chart_collection_count = alt.Chart(df_collection_count).mark_bar(color='#B7B8FD').encode(
    ...
    with top_collections_volume:
        ...
        chart_collection_volume = alt.Chart(df_collection_volume).mark_bar(color='#B7B8FD').encode(
    ...
    with daily_sales_count:
        ...
        daily_sales_count_area = daily_sales_count_base.mark_area(color='#B7B8FD').encode(
            y='Sales Count:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Count:Q', format=',')
    ...
    with daily_sales_volume:
        daily_sales_volume_base = alt.Chart(df_daily_sales_volume).encode(
            x=alt.X('Date:T', axis=alt.Axis(title=None)))
        daily_sales_volume_area = daily_sales_volume_base.mark_area(color='#B7B8FD').encode(
            y='Sales Volume:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
                alt.Tooltip('Sales Volume:Q', format='$,.2f')
    ...
    with daily_traders:
        ...
        daily_traders_area = daily_traders_base.mark_area(color='#B7B8FD').encode(
            y='Traders:Q',
            tooltip=[
                alt.Tooltip('Date:T'),
    ...
    with daily_buyer_seller:
        daily_buyer_seller_color = {'Buyer': '#B7B8FD', 'Seller': '#7FE0FE'}
    ...
    with daily_buyer_seller_ratio:
        daily_buyer_seller_ratio_chart = alt.Chart(df_daily_buyer_seller_ratio).mark_line(color='#B7B8FD').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
    ...
    with trader_group_sales_count:
        trader_group_sales_count_chart = alt.Chart(df_trader_group_sales_count).mark_bar(color='#B7B8FD').encode(
    ...
    with trader_group_sales_volume:
        trader_group_sales_volume_chart = alt.Chart(df_trader_group_sales_volume).mark_bar(color='#B7B8FD').encode(
    ...
    for item in json_trader_share:
        if (item['BLOCK_DATE'] >= '2022-04-25'):
            save_dict_trader_share["Date"].append(item["BLOCK_DATE"])
            save_dict_trader_share["% of Total Traders on Ethereum"].append(item["SUDOSWAP_SHARE"])
    ...
    with trader_share:
        trader_share_chart = alt.Chart(df_trader_share).mark_area(color='#B7B8FD').encode(
            x=alt.X("Date:T", axis=alt.Axis(title=None)),
